...
const UploadVedio = ({setshowUploadVedio}:Props) => {
   ...
   async function handleUpload(e:React.SyntheticEvent<HTMLFormElement>){
    ...
    try{
    //Got the presigned url and Final vedio url(vedio address in cloud) where the vedio is going to save in the bucket 
    const preSignedResponse = await axios.post("http://localhost:3000/getPresignedUrl");
    const {putUrl , finalVedioUrl} = preSignedResponse.data;
   
    //Upload video directly to R2
    const vedioUploadResponse = await axios.put(putUrl,video, {
      headers:{
        "Content-Type":video.type //Matlab content ka type kya hai , toh yeh ek vedio hai.
      },
      onUploadProgress(progressEvent) {
          const percent = Math.round((progressEvent.loaded*100)/progressEvent.total!) //we calculated here the total percentage of vedio uploaded
          setvedioUploadPercent(percent);
      },
    })

    console.log("Upload successful",vedioUploadResponse) ; 
    console.log("Public URL:" , finalVedioUrl);
    }catch(error){
      if(axios.isAxiosError(error)){
      const errorMessage = error.response?.data?.message;
      console.log(errorMessage)
      }else{
        console.log("Unexpected error occured in saving video to cloud")
      }
    }
   

    try{
      const thumbnailPreSignedUrl = await axios.post("http://localhost:3000/getThumbnailPresignedUrl",{fileType:image.type});
      const {putUrl , finalImageUrl} = thumbnailPreSignedUrl.data;
      //made the request to backend and got the permission and signedUrl

      const thumbnailUploadResponse = await axios.put(putUrl,image,{
        headers:{
          "Content-Type":image.type
        },
        onUploadProgress(progressEvent) {
          const percent = Math.round((progressEvent.loaded*100)/progressEvent.total!) //we calculated here the total percentage of thumbnail uploaded
          setimageUploadPercent(percent);
        },
      })
      //After the permission , we put the file in the cloud

      console.log("Public Thumbnail Url : " , finalImageUrl)
      ...
      console.log(errorMessage)
        ...
        console.log("Unexpected error occured in saving Thumbnail to cloud")
      }
    }
   ...
   const totalProgress = vedioUploadPercent * 0.5 + imageUploadPercent * 0.5  //yaha 0.5 se multiply ka matlab hai ki pure progress bar ka 50% is equivalent to 100% of the vedio and left 50% s equivalent to 50% of thumbnail upload .
